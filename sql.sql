...
-- 4.5 Refund de shipment individual (reemplaza fn_complete_dispute_refund — eliminada)
...
RETURNS TABLE(success BOOLEAN, error_message TEXT)
LANGUAGE plpgsql
SECURITY DEFINER -- El cron corre como postgres de forma segura
SET search_path TO public, pg_temp
AS $$
DECLARE
  v_status      TEXT;
...
-- API hardening: solo el cron de retorno (service_role) puede invocar este rpc logístico
...
-- 4.8 fn_seller_initiate_return_label (Guardia de Validación Pre-vuelo)
...
RETURNS TABLE(success BOOLEAN, error_message TEXT)
LANGUAGE plpgsql
SECURITY DEFINER -- ← Mantiene consistencia de permisos de lectura para el seller
SET search_path TO public, pg_temp
AS $$
DECLARE
  v_seller_id   UUID;
...
EXCEPTION WHEN OTHERS THEN
  -- FIX: Inyectamos el insert de telemetría para mantener consistencia absoluta
  -- con todo el sistema de diagnóstico de errores del proyecto.
  INSERT INTO public.system_logs (level, message, metadata)
...
-- API hardening: solo la Edge Function (service_role) puede invocar este pre-flight
...
-- Transición: waiting_return → return_shipped tras pago exitoso en Stripe
-- =========================================================================

CREATE OR REPLACE FUNCTION public.fn_log_return_payment(
    p_dispute_id UUID,
    p_stripe_id TEXT,
    p_amount NUMERIC
)
...
-- API hardening: solo stripe-webhooks (service_role) puede registrar pagos de retorno
REVOKE EXECUTE ON FUNCTION public.fn_log_return_payment(UUID, TEXT, NUMERIC) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.fn_log_return_payment(UUID, TEXT, NUMERIC) TO service_role;
