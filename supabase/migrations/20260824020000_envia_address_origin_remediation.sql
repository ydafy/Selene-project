...
CREATE OR REPLACE FUNCTION public.prevent_shipment_origin_snapshot_mutation()
RETURNS TRIGGER LANGUAGE plpgsql SET search_path = public, pg_temp AS $$
BEGIN
  IF OLD.origin_address IS NOT NULL AND (
    NEW.origin_address IS DISTINCT FROM OLD.origin_address
    OR NEW.origin_address_id IS DISTINCT FROM OLD.origin_address_id
  ) THEN RAISE EXCEPTION 'ORIGIN_SNAPSHOT_IMMUTABLE'; END IF;
  RETURN NEW;
END; $$;

DROP TRIGGER IF EXISTS shipments_origin_snapshot_immutable ON public.shipments;
CREATE TRIGGER shipments_origin_snapshot_immutable
  BEFORE UPDATE ON public.shipments
  FOR EACH ROW EXECUTE FUNCTION public.prevent_shipment_origin_snapshot_mutation();

REVOKE EXECUTE ON FUNCTION public.fn_claim_shipment_label(UUID, UUID) FROM PUBLIC, anon, authenticated;
DROP FUNCTION public.fn_claim_shipment_label(UUID, UUID);
CREATE FUNCTION public.fn_claim_shipment_label(
  p_shipment_id UUID, p_seller_id UUID, p_origin_address_id UUID
...
REVOKE EXECUTE ON FUNCTION public.fn_claim_shipment_label(UUID, UUID, UUID) FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION public.fn_claim_shipment_label(UUID, UUID, UUID) TO service_role;
