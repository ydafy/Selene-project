-- ============================================================================
-- Migration: single_modal_settlement_rpc
-- Created: 2026-07-03
-- Description: Deploys the Phase 4 single-modal multi-seller checkout
--              settlement RPC `fn_create_shipments_from_single_payment`.
--
--              The webhook (supabase/functions/stripe-webhooks/index.ts)
--              routes `payment_intent.succeeded` events whose metadata carries
--              `flow='single_modal_connect_checkout'` to this RPC, which
--              persists the order + ALL per-seller shipments + order_items
--              from the platform PaymentIntent's embedded (chunked) allocation
--              JSON in ONE transaction, idempotent on
...
--              so the webhook can reply 200 (retry-safe).
--
-- Prerequisite: single_modal_checkout_settlement migration (20260702000000)
--   which adds orders.stripe_charge_id, stripe_transfer_group,
--   payment_processing, payment_processing_reason and
--   shipments.stripe_transfer_id. This migration MUST run after that one so
--   the RPC body can write those columns.
--
-- Canonical SQL source: supabase/queries/orders/fn_create_shipments_from_single_payment.sql
--   The two files are kept byte-identical in the CREATE OR REPLACE FUNCTION
--   block; this migration is the deployable path (per the repo convention of
--   embedding RPCs in a self-contained migration, as done for
--   fn_reserve_products / fn_release_products), and the queries/ file is the
--   navigable source-of-truth for editors.
--
-- NOTE (manual step): after applying this migration, regenerate the shared
--   generated DB row types by running `bun db:types` at the repo root.
-- ============================================================================
