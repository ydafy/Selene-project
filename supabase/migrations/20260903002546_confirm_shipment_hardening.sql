...
CREATE TABLE IF NOT EXISTS public.shipment_completion_events (
  ...
  CONSTRAINT shipment_completion_events_shipment_id_key UNIQUE (shipment_id),
  CONSTRAINT shipment_completion_events_idempotency_key_key UNIQUE (idempotency_key),
  CONSTRAINT shipment_completion_events_source_check
    CHECK (source IN ('buyer', 'auto')),
  CONSTRAINT shipment_completion_events_actor_source_check CHECK (
    (source = 'buyer' AND actor_id IS NOT NULL) OR (source = 'auto' AND actor_id IS NULL)
  )
);

ALTER TABLE public.shipment_completion_events ENABLE ROW LEVEL SECURITY;
REVOKE ALL ON TABLE public.shipment_completion_events FROM PUBLIC, anon, authenticated;

CREATE INDEX IF NOT EXISTS idx_shipments_auto_completion_due
  ON public.shipments (delivered_at)
  WHERE status = 'delivered'
    AND buyer_confirmed_at IS NULL
    AND delivered_at IS NOT NULL;

DROP FUNCTION IF EXISTS public.fn_confirm_shipment_delivery(uuid);
DROP FUNCTION IF EXISTS public.fn_confirm_delivery(uuid);
