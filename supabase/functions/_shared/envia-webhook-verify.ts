...
export interface VerifyEnviaWebhookInput {
  headers: Headers;
  rawBody: string;
  secret: string;
  now?: number;
...
export async function verifyEnviaWebhook({
  headers,
  rawBody,
  secret,
  now = Date.now(),
}: VerifyEnviaWebhookInput): Promise<boolean> {
  ...
  if (eventAt === null || Math.abs(now - eventAt) > replayWindowMs)
    return false;
