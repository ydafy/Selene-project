...
describe('Envia destination schemas', () => {
  it('rejects incomplete provider-required listing references and invalid state codes', () => {
    expect(
      listingReferenceDestinationSchema.safeParse({
        ...validDestination,
        phone: undefined,
      }).success,
    ).toBe(false);
  ...
  it('accepts a complete listing reference and rejects the same invalid state for buyer snapshots', () => {
    expect(
      listingReferenceDestinationSchema.safeParse(validDestination).success,
    ).toBe(true);
    expect(
      buyerDestinationSnapshotSchema.safeParse({
        ...validDestination,
        state: 'MX',
      }).success,
    ).toBe(false);
...
describe('Envia runtime configuration', () => {
  it('requires an explicit supported mode with its selected key and HTTPS URL', () => {
    expect(
      resolveEnviaRuntimeConfiguration({ ENVIA_MODE: undefined }),
    ).toBeNull();
    expect(
      resolveEnviaRuntimeConfiguration({
        ENVIA_MODE: 'sandbox',
        ENVIA_API_KEY_SANDBOX: 'key',
        ENVIA_API_URL_SANDBOX: 'http://envia.example',
      }),
    ).toBeNull();
  });
  ...
  it('accepts the selected HTTPS configuration and test-only localhost seam', () => {
    expect(
      resolveEnviaRuntimeConfiguration({
        ENVIA_MODE: 'production',
        ENVIA_API_KEY_PROD: 'key',
      ...
      resolveEnviaRuntimeConfiguration(
        {
          ENVIA_MODE: 'sandbox',
          ENVIA_API_KEY_SANDBOX: 'key',
          ENVIA_API_URL_SANDBOX: 'http://localhost:8787',
        },
...
describe('Envia generation outcome classification', () => {
  it('releases only proven provider rejections', () => {
    expect(
      classifyEnviaGenerateOutcome({
        status: 422,
        body: { error: 'invalid destination' },
      }),
    ).toBe('retryable_rejected');
    expect(
      classifyEnviaGenerateOutcome({
        status: 400,
  ...
  it('fails closed for timeouts, connection loss, and malformed or contradictory responses', () => {
    expect(classifyEnviaGenerateOutcome({ kind: 'timeout' })).toBe(
      'orphan_pending',
    );
    expect(classifyEnviaGenerateOutcome({ kind: 'connection_lost' })).toBe(
      'orphan_pending',
    );
    expect(
      classifyEnviaGenerateOutcome({
        status: 201,
        body: { trackingNumber: 'TRACK-1' },
      }),
    ).toBe('orphan_pending');
    expect(
      classifyEnviaGenerateOutcome({
        status: 422,
        body: { trackingNumber: 'TRACK-1', error: 'invalid destination' },
      }),
    ).toBe('orphan_pending');
  });
});
...
describe('Envia shipment configuration', () => {
  it('builds the sole V1 quote policy for Paquetexpress Ground in MXN', () => {
    expect(buildEnviaQuoteConfiguration()).toEqual({
  ...
  it('fails closed for any configured carrier or service outside the sole label policy', () => {
    ...
    expect(
      buildEnviaShipmentConfiguration({ ...base, envia_carrier: 'Estafeta' }),
    ).toBeNull();
    expect(
      buildEnviaShipmentConfiguration({ ...base, envia_service: 'Express' }),
    ).toBeNull();
  });
...
describe('Paquetexpress ground rate policy', () => {
  ...
  it('fails closed for alternate carriers, services, duplicate matches, and invalid currency', () => {
    expect(
      selectPaquetexpressGroundRate([{ ...exactRate, carrier: 'Estafeta' }]),
    ).toBeNull();
    expect(
      selectPaquetexpressGroundRate([{ ...exactRate, service: 'Express' }]),
    ).toBeNull();
    expect(
      selectPaquetexpressGroundRate([
        exactRate,
        { ...exactRate, quoteId: 'quote-2' },
      ]),
    ).toBeNull();
    expect(
      selectPaquetexpressGroundRate([{ ...exactRate, currency: 'USD' }]),
    ).toBeNull();
  });

  it('projects the sole accepted rate into the public quote boundary', () => {
    expect(buildPaquetexpressGroundQuote([exactRate])).toEqual({
      carrier: 'Paquetexpress',
      service: 'ground',
      price: 168.75,
      estimated_days: 3,
    ...
    expect(
      buildPaquetexpressGroundQuote([{ ...exactRate, currency: 'USD' }]),
    ).toBeNull();
  });
});
