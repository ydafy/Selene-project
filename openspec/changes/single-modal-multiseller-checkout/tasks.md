...
- [ ] 7.1 Test: PI has `transfer_group`, no `transfer_data.destination`
- [ ] 7.2 Test: webhook creates order+all shipments in one txn; duplicate PI idempotent
- [ ] 7.3 Test: release creates Transfer before Payout; skips if `stripe_transfer_id` exists; rejects missing `transfer_group`
- [ ] 7.4 Test: PI success + DB fail → `payment_processing=true` shell, retry recovers
- [ ] 7.5 Test: legacy per-seller flow still works via `flow` metadata toggle
