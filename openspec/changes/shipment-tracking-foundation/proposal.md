...
|---|---|---|
| `supabase/migrations/` | New | Ledger, events, RLS, indexes, atomic RPC. |
| `supabase/functions/envia-webhook/` | New | HMAC-verified Envia receiver. |
| `supabase/functions/track-{shipments,returns}/` | Modified | Reconciliation and deduped event persistence. |
| `apps/frontend/.../orders/` | Modified | Normalized chronological timeline and corrected Envia URL. |
| `openspec/specs/shipments/` | Modified | Shipment tracking lifecycle contract. |

## Risks

| Risk | Likelihood | Mitigation |
|---|---|---|
| Duplicate/replayed callbacks | Med | HMAC timestamp validation and unique delivery ledger. |
| Missed carrier events | Med | Shared polling reconciliation and event deduplication. |
| Invalid status regression | Low | Transactional lock and conditional transition guard. |
...
- Applied shipment hardening/foundation migrations and Envia server-side credentials.
- Envia signed `tracking.simple` webhook registration.

## Success Criteria

- [ ] Carrier events produce deduplicated, chronological, shipment-scoped timelines.
- [ ] Only carrier custody advances `preparing` to `shipped`; cancellations cannot regress.
- [ ] Webhook and polling reconcile without duplicate events; Envia links open the public tracking page.
